...
| 名義 | 状態 |
|---|---|
| **Level Trim** | 全6曲。#1 `Idle Loop`・#2 `Warm Cache` は審査中（#1リリース日9/6）。**ジャケット画像は全6曲3000×3000化完了（2026-09-08）**。**残り：Thread Pool再生成の判断／RouteNote入稿** |
| **Ongaku Toshokan** | 京都BGM、**単曲×10本**（各1時間超）。MP3スペック全曲合格（320kbps/44.1kHz）。01〜06審査中（04〜06は2026-09-12入稿、アシスタントが入稿作業を代行）。**ジャケットは04〜10の3000×3000化完了（2026-09-08）**。**残り：07〜10の入稿** |
| **Vermilion Gate** | ファンタジー和風・エピック系、既存YouTube公開曲70曲をRouteNoteでも配信。全曲棚卸し・音源変換・タイトル確定・Instrumentalフラグ設定済み。**動シリーズ40曲（D1〜D4）はジャケット3000×3000化完了（2026-09-08）**。静シリーズ30曲はGeminiプロンプト用意済み・画像生成待ち。入稿カレンダー9/12〜10/10分作成済み（音源・ジャケット実体コピー配置済み）。**残り：名義の直接重複確認／静シリーズの画像生成／YouTube Content ID解除の徹底** |
| **Hollow Rotor** | Phonk/ジムワークアウト向けインストBGM。#1 `Torque Lock`（150bpm）完成・リスニング確認OK。**#2 `Rev Limiter`（160bpm）は音源・ジャケットとも完成（2026-09-12）、リスニング確認は未実施**。入稿は他プロジェクトと同じ**土曜バッチ**で実施予定。**残り：名義の直接重複確認／#2のリスニング確認／RouteNote入稿** |
