...
| 名義 | 状態 |
|---|---|
| **Level Trim** | 全6曲。#1 `Idle Loop`・#2 `Warm Cache`・#3 `Long Poll` は審査中。**ジャケット画像は全6曲3000×3000化完了（2026-09-08）**。**残り：Thread Pool再生成の判断／Backpressure・Hot Pathの入稿（下記の一括入稿方針に沿って実施）** |
| **Ongaku Toshokan** | 京都BGM、**単曲×10本**（各1時間超）。MP3スペック全曲合格（320kbps/44.1kHz）。01〜06審査中。**ジャケットは04〜10の3000×3000化完了（2026-09-08）**。**残り：07〜10の入稿（下記の一括入稿方針に沿って実施）** |
| **Vermilion Gate** | ファンタジー和風・エピック系、既存YouTube公開曲70曲をRouteNoteでも配信。全曲棚卸し・音源変換・タイトル確定・Instrumentalフラグ設定済み。**動シリーズ40曲（D1〜D4）はジャケット3000×3000化完了（2026-09-08）**。静シリーズ30曲はGeminiプロンプト用意済み・画像生成待ち。**残り：名義の直接重複確認（完了後、D1・D2 20曲を独立の高速レーンで一括入稿）／静シリーズの画像生成／YouTube Content ID解除の徹底** |
| **Hollow Rotor** | Phonk/ジムワークアウト向けインストBGM。Track #1〜4は音源・ジャケットとも完成、リスニング確認済み（#2は声がわずかに残るがユーザー判断で採用）。**残り：Track #1〜4の入稿（下記の一括入稿方針に沿って実施）／Track #5〜6の設計・生成** |

**⚠️ Hollow Rotorはローカルセッション単独で作られ、本リポジトリへの同期は2026-09-07に実施。以降も複数セッションから並行更新されている。** 経緯は `docs/hollow-rotor/master.md` 参照。

**⚠️ RouteNoteの審査待ちは現在27〜29営業日（約6週間）。** 入稿カレンダーはこれを織り込んで組むこと（`docs/policy.md` 参照）。

**⚠️ 2026-09-17〜：入稿ペースを上げる方針に変更（`docs/policy.md` 5章）。** 在庫（完成済み・未入稿の曲）が数ヶ月分溜まっていたため、①**入稿（審査キュー投入）と公開（Sales Start Date）を切り離し**、在庫はまとめて一括入稿・公開日だけ従来の週2〜3件ペースで分散、②**Vermilion Gateは既存YouTube公開済みカタログの移植なので、他3名義とは別の高速レーンでまとめて入稿してよい**、の2点。Sales Start Dateを審査完了前に指定した場合の挙動は未検証のまま本番投入している（ユーザー判断で検証省略）。各`docs/<名義>/master.md`に曲ごとのSales Start Dateスケジュールを記載済み。
...
**このプロジェクトに関わるファイルは、ドキュメント・音源・画像を問わず削除しない。** 不要になったら削除ではなく、対象と同じ階層に `archive/` フォルダを作ってそこに移動する。

- リポジトリ内のドキュメント → 各 `docs/<project>/archive/` に移す（既存ルール、下記参照）
- ローカルの音源・ジャケット画像・入稿予定フォルダ等 → その場に `archive/` を作り、そこへ移動する（例：`RouteNote/入稿予定/2026-09-12/archive/`）
- 判断に迷う古いファイルも、確認せず削除しない。まず `archive/` へ退避する
...
- **この2つは新規ファイルのみ適用**（2026-08-30決定）。既存の音源・フォルダ（日本語名／`#`付き）はリネームしない
