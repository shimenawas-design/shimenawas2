...
### ⚠️ 運用方針の決定（2026-08-24）：推奨間隔より頻度を優先

上記「推奨リリース間隔4〜6週に1曲」（確度：中、二次情報）を踏まえたうえで、**Level Trim / Ongaku Toshokan とも「週2〜3件・土曜バッチ予約投稿」で進めることを決定**。

- **認識している代償**：4〜6週間隔より大幅に速いため、Discover Weekly等のアルゴリズム最適化（ジャンル分類の安定化に必要な「同一ジャンルで3〜5リリース」の効果が薄まる可能性）を犠牲にする
- **優先した理由**：カタログを早く積み上げる速度を優先。ユーザーの明示判断（2026-08-24）
- **効いてくる制約は変わらない**：RouteNoteの審査待ち（27〜29営業日）とDeezerの検出リスクは頻度に関わらず存在するため、そちらへの配慮（一次情報の入稿規約遵守、AI企業名を明記しない等）は従来通り必須
- 具体的な入稿カレンダーは各プロジェクトの`master.md`を参照
