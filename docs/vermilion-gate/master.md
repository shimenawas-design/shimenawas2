...
7. **ジャケット画像・アートワークの準備 — 進行中（2026-09-08）**。
   - **3000×3000化まで完了**：D1（全10曲）・D2（全10曲）。既存YouTubeサムネの黒帯／レターボックスを自動検出して正方形に切り出し（`jacket_3000.png`として`RouteNote/Vermilion Gate/`各曲フォルダおよび入稿カレンダー3週分に配置済み）
   - **候補画像のみ（Canva/切り出し未実施）**：D3・D4（計20曲、サムネ自体は既存確認済み）
   - **未確認**：静シリーズ（S1・S2は全曲、S3は4〜10）。S3の1〜3のみサムネ確認済みで切り出し未実施
   - Canvaでの人手リサイズは不要になった（自動切り出しで代替）
