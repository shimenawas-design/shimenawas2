...
- **既知の注意点**: Gemini生成画像には右下付近に小さな透かし（星マーク）が入ることがある（Track #1で発生）。周囲が単色背景の場合はその領域を背景色で塗りつぶして除去できる（ffmpeg/PILでの後処理）。ただし毎回出るわけではない（Track #2では透かしなしの結果だった）。生成のたびに右下を確認すること
